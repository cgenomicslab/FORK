...
| `ete_species_tree.py` | — | Standalone interactive ETE4 NCBI species tree, coloured by comparison group (Comparative tab) |
| `templates/compare.html` | — | Comparative "concept check" page (families present in one taxon group but absent in another) |
| `templates/profiling.html` | — | Continuous Presence/Absence → High-Resolution profiling page |
| `static/ete4_overrides/contextmenu.js` | — | Repo-served override of ETE4's right-click menu (trimmed + "Use branch for profiling"); served via the ETE4 proxy so the read-only conda env is untouched |
...
| `/profiling` | GET | Serve `profiling.html` |
| `/compare` | GET | Serve `compare.html` (Comparative "concept check" tab) |
...
| `/api/highres/download-tree` | POST | Download one built gene tree as a plain Newick file |
...
#### Comparative — "concept check" (`/compare`)

| Route | Method | Purpose |
|---|---|---|
| `/api/compare/families` | POST | Given two taxon groups (typed/clade IDs + optional trees), return families present in A-not-B, B-not-A, shared, and a heatmap |
| `/api/compare/family-proteins` | POST | Drill-down: protein accessions carrying a clicked family within one group |
| `/api/compare/species-tree` | POST | Render the groups' NCBI species tree as a static PNG, coloured by group |
| `/api/compare/ete-tree` | POST | Launch the interactive ETE4 species tree (`ete_species_tree.py`), coloured by group |
