...
Created: 2026-07-06 · Updated: 2026-07-13
...
**plain-Newick download in every tab**; **zero-width-tree fix**; **customised ETE4
context menu + tag-branch-to-profiling**; **new Comparative "concept check" tab**)

---

## Round 6 — Newick download, tree fixes, ETE4 menu, Comparative tab (2026-07-13)

### New Comparative ("concept check") tab
- **New tab `/compare`** (`templates/compare.html`, nav link in `base.html`). Answers questions
  like *"which protein families are present in Mucorales but absent in Human?"*
- **Inputs (both methods):** typed **taxon / clade IDs** (a clade like `4827` is expanded to its
  member species via `NCBITaxa.get_descendant_taxa`) **and/or** taxa derived from an **uploaded
  Newick tree** (leaf taxids). Two groups: A = "present in", B = "absent in". Optional e-value.
- **Report:** summary counts; **families present in A-not-B**, **B-not-A**, and **shared**
  (tables, CSV download); a **presence/absence heatmap** of the top differential families across
  group-A taxa (reuses `viz.draw_presence_absence_heatmap`); and a **species tree coloured by
  group** — static PNG *and* the **interactive ETE4 explorer** (green = A only, amber = B only,
  blue = both; capped at 150 taxa).
- **Protein drill-down:** clicking a family's **`n_proteins`** count lists that family's actual
  protein **accessions** (accession · taxon · organism · best e-value) for the clicked group, in a
  table with CSV download. Endpoint `POST /api/compare/family-proteins`; library helper
  `get_family_accessions` / `fetch_family_accessions` (added to `__all__`).
- **Backend (additive):** `GET /compare`; `POST /api/compare/families`,
  `/api/compare/family-proteins`, `/api/compare/species-tree`, `/api/compare/ete-tree`. New library
  helpers `get_families_for_taxa` / `fetch_families_for_taxa` (distinct HMM families per taxon set,
  with counts) and `get_db_taxa` / `fetch_db_taxa` (restrict a clade-expanded set to taxa actually
  in the DB) — both added to `__all__`. New standalone explorer script **`ete_species_tree.py`**
  (modelled on `ete_profile.py`).
- **Additive-only:** nothing existing was modified except a nav link in `base.html` and two
  `__all__` entries — a `git checkout` reverts the whole feature.
- **Fixes (same day):**
  - **Clade expansion missed species-level taxa.** `get_descendant_taxa` was called with
    `intermediate_nodes=False`, which returns only NCBI *leaf* taxa — but reference proteomes are
    keyed to *species* taxids, which are frequently *internal* NCBI nodes (they have sequenced-strain
    children). Result: a clade like Mucorales (4827) matched 0 DB taxa. Now uses
    `intermediate_nodes=True` (species **and** strain taxids). Harmless extra genus/family taxids
    simply don't match any proteins.
  - **Cross-thread NCBITaxa crash.** `_resolve_taxon_group` reused the module-level `NCBITaxa()`;
    its SQLite connection is **not thread-safe**, so under Flask's request threads the query raised
    "SQLite objects created in a thread can only be used in that same thread", was swallowed by a
    bare `except`, and the clade never expanded (only the literal taxid survived → "1 taxa resolved,
    0 in DB"). Now creates a **fresh `NCBITaxa()` per call**, matching every other endpoint. (The
    High-Resolution **auto-duplication** partition mode was checked and is already safe — it passes a
    fresh `NCBITaxa()` into `partition_by_duplication` per request.)
  - Error message now reports how many taxa were resolved from NCBI vs found in the DB, so a bad
    taxid or a genuine coverage gap is obvious.
  - **Heatmap capped to 60 taxa (rows).** A clade can expand to thousands of taxa; a
    several-thousand-row clustermap produced a PNG too large for the browser (it showed as a broken
    image). The heatmap now keeps the 60 taxa carrying the most of the differential families and
    shows a note; the full family tables are unaffected.

### Download the built tree as a plain, clean Newick — in every tab
- **Tree tab:** a **"Tree (Newick)"** button now shows in *every* viewer mode (previously only in
  the static-PNG section). Fixed the underlying gap: the ETE4-server and static branches now store
  `newick`/`tree_path` (via a shared `_built_newick()` helper) and `/api/job` reports `has_newick`
  for every viewer. Download filename cleaned to basename.
- **Profiling / High-res:** a per-Pfam **"Download Newick"** button (`POST
  /api/highres/download-tree`). Serves the raw IQ-TREE/FastTree `.treefile` — clean standard
  Newick, no NHX/iTOL sidecar data.

### "Cannot draw tree with width 0" → dendrogram fallback
- ETE4 raises this for zero-width trees (identical sequences, or topology-only NCBI trees). The
  per-branch guard now has a whole-tree fallback: if the tree is still ~zero width, it's drawn as a
  **uniform-branch dendrogram** so it always renders. Applied in `tree_from_db.py` (gene-tree
  preview) and the standalone `ete_highres_profile.py` / `ete_profile.py` viewers. Trees with real
  branch lengths are untouched.

### Customised ETE4 right-click menu (served from the repo, no conda write)
- The ETE4 context menu is trimmed (destructive Move/Sort/Convert/Remove items removed). Because the
  `ete4` package lives in a **read-only system conda env**, the customised `contextmenu.js` lives in
  the repo at **`static/ete4_overrides/contextmenu.js`** and is served through FORK's existing ETE4
  proxy (`ete4_static_proxy` intercepts `js/contextmenu.js`). Survives `ete4` upgrades; no root.

### Tag a branch in the ETE4 viewer → use it for profiling
- A **"📎 Use branch for profiling"** menu item posts the branch's node path to the parent page
  (`postMessage`; the viewer iframe is same-origin via the proxy). The Profiling/High-res page adds
  that path to the Pfam's **Node-path** list, reusing the click-to-select machinery. The viewer
  carries its Pfam via the iframe URL, captured into `window.__fork_pfam` before it's stripped from
  the URL (so ETE4 doesn't flag an unknown `pfam` param). The menu item only appears where a Pfam
  context exists (hidden on the Tree tab and the matrix/species-tree viewers).

**Verified:** all Python compiles; templates parse; new JS passes `node --check`; the app boots and
`/compare` returns 200 with the nav link live. The comparison set-difference/token flow was
unit-tested with a mocked DB (A-not-B / B-not-A / shared correct; bad token → 400). **The live-DB
queries and the interactive ETE4 species tree need the real database + ETE4 runtime to confirm
end-to-end.**
