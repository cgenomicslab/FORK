# UniProt Lab Manager — Green interface (test copy)
...
(real KPIs; Recent runs driven by the job registry; wider ETE4 explorers; **rebranded to
PhyloWave**; About tab; colormap + partition-lock + ETE4 domain-track fixes)
...
3. **Rebrand to PhyloWave** (`base.html`, `main.css`). New name with a minimal wave mark,
   ...
   footer updated to PhyloWave. (The earlier static four-squares PNG was removed.)
...
- **Logo = the plain black "P" of PhyloWave, with a cartoon wave wrapping around it.** No
  separate icon: the first letter of the **PhyloWave** wordmark is the ordinary black serif P,
